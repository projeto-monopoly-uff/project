...
*/
popupJogadores = '<div class="popup" id="jogadores-na-celular"> </div>'
...
for (var i = 1; i < 12; i++) {
    for (var j = 1; j < 12; j++) {
        ...
        if (i == 1) {
            $(".grid-container").append(`<div class="grid-item limit-row" id="celula${celula}">${popupJogadores}</div>`);
            celula++;
        }
        //Criando a última fileira
            ...
            if (j == 1) {
                ...
                $(".grid-container").append(`<div class="grid-item limit-col" id="celula${celula}">${popupJogadores}</div>`);
                diferenca++;
            //Célula do fim da fileira, depois do centro
            } else if (j == 11) {
                celula = celula + diferenca;
                $(".grid-container").append(`<div class="grid-item limit-col" id="celula${celula - 1}">${popupJogadores}</div>`);
                diferenca++;
